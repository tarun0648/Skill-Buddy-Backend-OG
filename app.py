# app.py (UPDATED - Added Interview System Integration + OTP/Phone Authentication Support)
...
os.makedirs('data', exist_ok=True)  # NEW: For interview system data files
...
# NEW: Import and register interview system blueprints
try:
    from routes.interview_routes import interview_bp
    app.register_blueprint(interview_bp, url_prefix='/api/interview')
    print("✓ Interview system routes loaded")
except ImportError as e:
    print(f"✗ Interview system routes failed: {e}")
    interview_bp = None

try:
    from routes.jd_interview_routes import jd_interview_bp
    app.register_blueprint(jd_interview_bp, url_prefix='/api/jd-interview')
    print("✓ JD-based interview routes loaded")
except ImportError as e:
    print(f"✗ JD-based interview routes failed: {e}")
    jd_interview_bp = None

try:
    from routes.status_routes import status_bp
    app.register_blueprint(status_bp, url_prefix='/api/status')
    print("✓ Status routes loaded")
except ImportError as e:
    print(f"✗ Status routes failed: {e}")
    status_bp = None
...
# Updated status endpoint with OTP service status and interview system
    ...
    try:
        stats = {
            ...
            'features_available': {
                ...
                'interview_system': interview_bp is not None,  # NEW
                'jd_based_interviews': jd_interview_bp is not None  # NEW
            },
            'profile_completion': {
        ...
        # NEW: Check interview system status
        try:
            # Check OpenAI configuration
            openai_api_key = os.environ.get('OPENAI_API_KEY')
            openai_assistant_id = os.environ.get('OPENAI_ASSISTANT_ID')
            
            # Check interview data files
            interview_questions_exist = os.path.exists('data/interview_questions.json')
            jd_questions_exist = os.path.exists('data/jd-predefined.json')
            
            stats['interview_system'] = {
                'enabled': interview_bp is not None,
                'jd_interviews_enabled': jd_interview_bp is not None,
                'openai_configured': bool(openai_api_key and openai_assistant_id),
                'interview_questions_available': interview_questions_exist,
                'jd_questions_available': jd_questions_exist,
                'ai_generation_available': bool(openai_api_key),
                'video_upload_support': True,
                'features': {
                    'role_based_interviews': interview_bp is not None,
                    'jd_based_interviews': jd_interview_bp is not None,
                    'ai_question_generation': bool(openai_api_key),
                    'answer_analysis': bool(openai_api_key),
                    'interview_history': True,
                    'skills_assessment': jd_interview_bp is not None,
                    'compatibility_scoring': jd_interview_bp is not None
                }
            }
        except Exception as e:
            stats['interview_system'] = {'enabled': False, 'error': str(e)}
        ...
        # NEW: Try to get interview statistics
        if interview_bp and db:
            try:
                ...
                # Simple count without complex queries
                try:
                    sessions_ref = db.collection('interview_sessions').limit(1000)
                    sessions = sessions_ref.get()
                    
                    interview_stats['total_sessions'] = len(sessions)
                    
                    for session in sessions:
                        session_data = session.to_dict()
                        assessment_type = session_data.get('assessment_type', 'role_based')
                        
                        if assessment_type == 'jd_based':
                            interview_stats['jd_based_sessions'] += 1
                        else:
                            interview_stats['role_based_sessions'] += 1
                        
                        if session_data.get('status') == 'completed':
                            interview_stats['completed_sessions'] += 1
                    
                except Exception as e:
                    logger.warning(f"Could not get detailed interview stats: {e}")
                
                stats['interview_statistics'] = interview_stats
                
            except Exception as e:
                stats['interview_error'] = str(e)
...
def health_check():
    return jsonify({
        'message': 'Skill Buddy API with Interview System & Phone OTP Authentication is running',
        'timestamp': datetime.datetime.now(datetime.timezone.utc).isoformat(),
        'status': 'healthy',
        'version': '5.0.0',  # Updated version for interview system
        'features': [
            'Resume Processing',
            'User Management', 
        ...
        'interview_features': {  # NEW
            'role_based_interviews': 'Traditional interviews based on job roles and experience levels',
            'jd_based_interviews': 'AI-generated questions from actual job descriptions',
            'ai_question_generation': 'Dynamic follow-up questions using OpenAI GPT',
            'video_interview_support': 'Optimized for video responses (no code writing)',
            'answer_analysis': 'Real-time assessment of response quality',
            'interview_history': 'Complete session tracking and analytics',
            'skills_assessment': 'JD-based skills matching and scoring',
            'compatibility_scoring': 'Candidate-job fit percentage calculation'
        },
        'interview_types': {  # NEW
            'software_engineer': 'Questions for software development roles',
            'engineering_manager': 'Leadership and technical management questions',
            'data_scientist': 'Data science and machine learning focused',
            'product_manager': 'Product strategy and management questions',
            'devops_engineer': 'Infrastructure and deployment questions',
            'custom_jd': 'Questions generated from any job description'
        },
...
# NEW: Test interview system endpoint
@app.route('/api/test-interview-system', methods=['GET'])
@auth_required
def test_interview_system():
    """Test endpoint to check interview system integration"""
    try:
        user_id = request.user_id
        ...
        # Test interview service import
        interview_service_available = False
        jd_interview_service_available = False
        assistant_client_available = False
        
        try:
            from services.interview_service import InterviewService
            interview_service_available = True
        except ImportError:
            pass
        
        try:
            from services.jd_interview_service import JDInterviewService
            jd_interview_service_available = True
        except ImportError:
            pass
        
        try:
            from services.assistant_client import AssistantClient
            assistant_client_available = True
        except ImportError:
            pass
        
        return jsonify({
            'message': 'Interview system integration test',
            'user_id': user_id,
            'timestamp': datetime.datetime.now(datetime.timezone.utc).isoformat(),
            'configuration': {
                'openai_api_key_configured': bool(openai_api_key),
                'openai_assistant_id_configured': bool(openai_assistant_id),
                'interview_questions_file_exists': interview_questions_exist,
                'jd_questions_file_exists': jd_questions_exist
            },
            'services_available': {
                'interview_service': interview_service_available,
                'jd_interview_service': jd_interview_service_available,
                'assistant_client': assistant_client_available,
                'firebase_service': True
            },
            'routes_available': {
                'interview_routes': interview_bp is not None,
                'jd_interview_routes': jd_interview_bp is not None
            },
            'features_status': {
                'role_based_interviews': interview_bp is not None and interview_service_available,
                'jd_based_interviews': jd_interview_bp is not None and jd_interview_service_available,
                'ai_question_generation': bool(openai_api_key) and assistant_client_available,
                'video_upload': True,
                'interview_history': True,
                'skills_assessment': jd_interview_service_available,
                'compatibility_scoring': jd_interview_service_available
            },
            'setup_recommendations': get_interview_setup_recommendations(
                bool(openai_api_key), 
                bool(openai_assistant_id),
                interview_questions_exist,
                jd_questions_exist,
                interview_service_available,
                jd_interview_service_available
            )
        }), 200
        
    except Exception as e:
        logger.error(f"Interview system test error: {e}")
        return jsonify({
            'error': 'Interview system test failed',
            'details': str(e)
        }), 500

def get_interview_setup_recommendations(openai_key, assistant_id, questions_file, jd_file, service1, service2):
    """Generate setup recommendations for interview system"""
    recommendations = []
    
    if not openai_key:
        recommendations.append("Set OPENAI_API_KEY environment variable for AI question generation")
    if not assistant_id:
        recommendations.append("Set OPENAI_ASSISTANT_ID environment variable for AI integration")
    if not questions_file:
        recommendations.append("Create data/interview_questions.json file with role-based questions")
    if not jd_file:
        recommendations.append("Create data/jd-predefined.json file with JD-based questions")
    if not service1:
        recommendations.append("Ensure services/interview_service.py is properly configured")
    if not service2:
        recommendations.append("Ensure services/jd_interview_service.py is properly configured")
    
    if not recommendations:
        recommendations.append("Interview system is fully configured and ready to use!")
    
    return recommendations

# Test OTP service endpoint
...
# NEW: Debug interview system endpoint
@app.route('/api/debug/interview-system', methods=['GET'])
def debug_interview_system():
    """Debug interview system configuration and data files"""
    try:
        import json
        ...
        # Check data files
        interview_questions_path = 'data/interview_questions.json'
        jd_questions_path = 'data/jd-predefined.json'
        
        interview_questions_exist = os.path.exists(interview_questions_path)
        jd_questions_exist = os.path.exists(jd_questions_path)
        ...
        if interview_questions_exist:
            try:
                with open(interview_questions_path, 'r') as f:
                    interview_data = json.load(f)
                    interview_questions_count = len(interview_data)
                    sample_roles = list(set([q.get('role', 'Unknown') for q in interview_data[:10]]))
            except Exception as e:
                interview_questions_exist = f"Error reading file: {e}"
        
        if jd_questions_exist:
            try:
                with open(jd_questions_path, 'r') as f:
                    jd_data = json.load(f)
                    jd_questions_count = len(jd_data)
            except Exception as e:
                jd_questions_exist = f"Error reading file: {e}"
        ...
        try:
            from services.interview_service import InterviewService
            services_status['interview_service'] = 'Available'
        except ImportError as e:
            services_status['interview_service'] = f'Import failed: {e}'
        
        try:
            from services.jd_interview_service import JDInterviewService
            services_status['jd_interview_service'] = 'Available'
        except ImportError as e:
            services_status['jd_interview_service'] = f'Import failed: {e}'
        
        try:
            from services.assistant_client import AssistantClient
            services_status['assistant_client'] = 'Available'
        except ImportError as e:
            services_status['assistant_client'] = f'Import failed: {e}'
        
        try:
            from services.firebase_service import FirebaseService
            services_status['firebase_service'] = 'Available'
        except ImportError as e:
            services_status['firebase_service'] = f'Import failed: {e}'
        ...
        if not openai_assistant_id:
            recommendations.append("Set OPENAI_ASSISTANT_ID environment variable")
        if not interview_questions_exist:
            recommendations.append("Create data/interview_questions.json file")
        if not jd_questions_exist:
            recommendations.append("Create data/jd-predefined.json file")
        
        for service, status in services_status.items():
            if 'failed' in status:
                recommendations.append(f"Fix {service} import issue")
        
        if not recommendations:
            recommendations.append("Interview system is fully configured!")
        
        return jsonify({
            'message': 'Interview system debug information',
            'timestamp': datetime.datetime.now(datetime.timezone.utc).isoformat(),
            'configuration': {
                'openai_api_key_configured': bool(openai_api_key),
                'openai_assistant_id_configured': bool(openai_assistant_id),
                'openai_key_preview': f"{openai_api_key[:8]}..." if openai_api_key else None,
                'assistant_id_preview': f"{openai_assistant_id[:12]}..." if openai_assistant_id else None
            },
            'data_files': {
                'interview_questions_exist': interview_questions_exist,
                'jd_questions_exist': jd_questions_exist,
                'interview_questions_count': interview_questions_count,
                'jd_questions_count': jd_questions_count,
                'sample_roles': sample_roles[:5]  # Show first 5 roles
            },
            'services': services_status,
            'routes': routes_status,
            'database': {
                'firebase_connected': db is not None,
                'collections_available': ['interview_history', 'interview_sessions', 'interview_summaries']
            },
            'recommendations': recommendations,
            'setup_status': {
                'ready_for_role_based_interviews': all([
                    openai_api_key, openai_assistant_id, interview_questions_exist, 
                    services_status.get('interview_service') == 'Available',
                    interview_bp is not None
                ]),
                'ready_for_jd_interviews': all([
                    openai_api_key, openai_assistant_id, jd_questions_exist,
                    services_status.get('jd_interview_service') == 'Available',
                    jd_interview_bp is not None
                ])
            }
        }), 200
        
    except Exception as e:
        logger.error(f"Interview system debug error: {e}")
        return jsonify({
            'error': 'Debug failed',
            'details': str(e)
        }), 500
...
if __name__ == '__main__':
    ...
    print("=== Skill Buddy API with Interview System & Phone OTP Authentication Starting ===")
    print(f"Port: {port}")
    print(f"Debug: {debug}")
    print("Features:")
    ...
    print("  ✓ Phone OTP Authentication")
    print("  ✓ Interview System (NEW)")
    print("  ✓ JD-based Interviews (NEW)")
    print("")
    ...
    print("  • Role-based interviews (Software Engineer, Manager, etc.)")
    print("  • JD-based interviews (Custom questions from job descriptions)")
    print("  • AI question generation using OpenAI GPT")
    print("  • Video interview optimization (no code writing)")
    print("  • Real-time answer analysis and feedback")
    print("  • Interview history and analytics")
    print("  • Skills assessment and compatibility scoring")
    print("")
    print("OTP Features:")
    ...
    print("")
    print("Interview System API Endpoints:")
    print("  • /api/interview/* - Role-based interview endpoints")
    print("  • /api/jd-interview/* - JD-based interview endpoints")
    print("  • /api/test-interview-system - Integration test endpoint")
    print("  • /api/debug/interview-system - Debug configuration")
    print("========================================================")
