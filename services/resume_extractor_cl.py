...
def extract_text_from_pdf(pdf_path):
    ...
    # Method 1: Try PyPDF2 with different settings
    try:
    ...
    # Method 2: Try with different PDF reader settings
    try:
        with open(pdf_path, 'rb') as pdf_file:
            pdf_reader = PyPDF2.PdfReader(pdf_file)
            text = ""
        ...
        if text.strip():
            logger.info(f"Successfully extracted {len(text)} characters using alternative method")
            return text
    except Exception as e:
...
def extract_resume_details(resume_file_path):
    ...
    try:
        ...
        pdf_text = extract_text_from_pdf(resume_file_path)
        
        if not pdf_text.strip():
            logger.error("No text could be extracted from the PDF")
            return {"error": "No text could be extracted from the PDF. The file may be corrupted, password-protected, or contain only images."}, None, 0
