...
class GitHubAnalyzerService:
    """Service for analyzing GitHub profiles using Claude AI and GitHub API with enhanced parallel processing"""
    ...
    def update_user_github_status(self, user_id: str, github_username: str, analysis_completed: bool = False):
        """Update user's GitHub status and recalculate profile completion"""
        try:
            ...
            if analysis_completed:
                user_model.update_github_analysis_status(user_id, True)
            
            logger.info(f"Updated GitHub status for user {user_id}: {github_url}")
        except Exception as e:
            logger.error(f"Error updating user GitHub status: {e}")
    
    def start_github_analysis(self, user_id: str, github_username: str, user_profile: Dict[str, Any]) -> Tuple[str, bool]:
        """Start asynchronous GitHub profile analysis using task manager"""
        try:
            ...
            task_id = task_manager.start_task(
                ...
                user_id=user_id,
                task_data=task_data,
                processing_func=self._analyze_github_task,
                analysis_id=analysis_id,
                github_username=github_username,
                user_profile=user_profile
            )
            ...
            logger.info(f"Started GitHub analysis for user {user_id}, task_id: {task_id}, analysis_id: {analysis_id}")
            return task_id, True
    ...
    def _analyze_github_task(self, task_id: str, user_id: str, analysis_id: str, github_username: str, user_profile: Dict[str, Any]) -> Dict[str, Any]:
        """Analyze GitHub profile asynchronously - called by task manager"""
        try:
            logger.info(f"Starting GitHub analysis task {task_id} for analysis {analysis_id}")
            ...
            if not github_username or not github_username.strip():
                error_message = "GitHub username is required for analysis"
            ...
            if not github_username.replace('-', '').replace('_', '').isalnum():
                error_message = "Please provide a valid GitHub username"
                self.analysis_model.update_analysis_status(analysis_id, 'failed', error_message)
                logger.error(f"GitHub analysis failed for {analysis_id}: Invalid username format")
                return {'status': 'failed', 'error': error_message}
            ...
            try:
                ...
                error_message = f"Failed to fetch GitHub data: {str(e)}"
                self.analysis_model.update_analysis_status(analysis_id, 'failed', error_message)
                logger.error(f"GitHub analysis failed for {analysis_id}: {error_message}")
                return {'status': 'failed', 'error': error_message}
            ...
            if 'error' in github_data:
                ...
                if "not found" in error_message.lower():
                    error_message = f"GitHub profile '{github_username}' not found. Please check the username and try again."
                elif "rate limit" in error_message.lower():
                    error_message = "GitHub API rate limit exceeded. Please try again in a few minutes."
                elif "private" in error_message.lower():
                    error_message = "This GitHub profile is private. Please make it public to enable analysis."
            ...
            return {
                ...
                'grade': grade,
                'github_stats': github_data.get('user_stats', {})
            }
            
        except Exception as e:
            logger.error(f"Error in GitHub analysis task: {e}")
            error_message = f"Unexpected error during analysis: {str(e)}"
            self.analysis_model.update_analysis_status(analysis_id, 'failed', error_message)
            return {'status': 'failed', 'error': error_message}
    
    def get_processing_status(self, user_id: str) -> Optional[Dict[str, Any]]:
        """Get processing status of user's latest GitHub analysis task"""
        try:
            # Get latest GitHub task for user
            latest_task = task_manager.get_latest_task(user_id, 'github')
            ...
            return {
                ...
                'github_username': latest_task.get('task_data', {}).get('github_username', ''),
                'is_active': status_data.get('is_active', False)
            }
            
        except Exception as e:
            logger.error(f"Error getting GitHub processing status: {e}")
            return None
    
    def get_analysis_results(self, user_id: str) -> Optional[Dict[str, Any]]:
        """Get complete GitHub analysis results for user's latest analysis"""
        try:
            # Get latest GitHub task for user
            latest_task = task_manager.get_latest_task(user_id, 'github')
            ...
            if latest_task.get('status') != 'completed':
                return {
                    ...
                    'message': 'GitHub analysis not completed yet'
                }
            ...
            logger.error(f"Error getting GitHub analysis results: {e}")
            return None
