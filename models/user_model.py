...
class UserModel:
    ...
    def update_linkedin_link(self, user_id: str, linkedin_url: str) -> bool:
        """Update user's LinkedIn link and recalculate profile completion"""
        try:
            logger.info(f"Updating LinkedIn link for user {user_id}: {linkedin_url}")
            ...
            # Update the profile with new LinkedIn link
            current_profile['linkedin_link'] = linkedin_url
            ...
            success = self.update_user(user_id, update_data)
            if success:
                logger.info(f"Successfully updated LinkedIn link for user {user_id}: {linkedin_url}, completion: {old_completion}% -> {new_completion}%")
    ...
    def update_github_link(self, user_id: str, github_url: str) -> bool:
        """Update user's GitHub link and recalculate profile completion"""
        try:
            logger.info(f"Updating GitHub link for user {user_id}: {github_url}")
            ...
            # Update the profile with new GitHub link
            current_profile['github_link'] = github_url
            ...
            success = self.update_user(user_id, update_data)
            if success:
                logger.info(f"Successfully updated GitHub link for user {user_id}: {github_url}, completion: {old_completion}% -> {new_completion}%")
    ...
    def update_linkedin_analysis_status(self, user_id: str, analysis_completed: bool) -> bool:
        """Update user's LinkedIn analysis status"""
        try:
            # This method can be used to track if LinkedIn analysis has been completed
            # For now, we'll just log it
            logger.info(f"LinkedIn analysis status updated for user {user_id}: {analysis_completed}")
            return True
        except Exception as e:
            logger.error(f"Error updating LinkedIn analysis status: {e}")
            return False
    
    def update_github_analysis_status(self, user_id: str, analysis_completed: bool) -> bool:
        """Update user's GitHub analysis status"""
        try:
            # This method can be used to track if GitHub analysis has been completed
            # For now, we'll just log it
            logger.info(f"GitHub analysis status updated for user {user_id}: {analysis_completed}")
            return True
        except Exception as e:
            logger.error(f"Error updating GitHub analysis status: {e}")
            return False
    
    def add_xp(self, user_id: str, xp_amount: int, reason: str = "") -> bool:
        ...
        try:
            ...
            # Add to badges if level increased
            current_level = user.get('xp', {}).get('level', 1)
            if new_level > current_level:
                badges = user.get('xp', {}).get('badges', [])
                level_badge = f"Level {new_level}"
                if level_badge not in badges:
                    badges.append(level_badge)
                    update_data['xp.badges'] = badges
            
            success = self.update_user(user_id, update_data)
            if success:
                logger.info(f"Added {xp_amount} XP to user {user_id} (reason: {reason}). New total: {new_total_xp}, Level: {new_level}")
    ...
    def update_sso_connection(self, user_id: str, provider: str, profile_data: Dict[str, Any], access_token: str = None) -> bool:
        """Update user's SSO connection data"""
        try:
            update_data = {
                f'{provider}_connected': True,
                f'{provider}_profile_data': profile_data,
                'updated_at': firestore.SERVER_TIMESTAMP
           }
        
            if access_token:
                update_data[f'{provider}_access_token'] = access_token
        
            return self.update_user(user_id, update_data)
        
        except Exception as e:
            logger.error(f"Error updating SSO connection: {e}")
            return False

    def get_sso_connections(self, user_id: str) -> Dict[str, Any]:
        """Get user's SSO connections status"""
        try:
            user = self.get_user_by_id(user_id)
            if not user:
                return {}
            ...
            return {
                ...
                'google_connected': user.get('sso_provider') == 'google',
                'primary_auth_method': user.get('sso_provider', 'email'),
                'linkedin_data_available': bool(user.get('linkedin_profile_data')),
                'github_data_available': bool(user.get('github_profile_data'))
            }
        
        except Exception as e:
            logger.error(f"Error getting SSO connections: {e}")
            return {}
