...
class PortfolioAnalysisModel:
    ...
    def get_user_analysis_summary(self, user_id: str) -> List[Dict[str, Any]]:
        """Get analysis summary for a user (optimized for listing)"""
        try:
            query = self.db.collection(self.collection_name)\
                .where('user_id', '==', user_id)\
            ...
            for doc in docs:
                ...
                analysis_summary = {
                    ...
                    'has_extracted_data': bool(analysis_data.get('extracted_data_json')),
                    'portfolio_url': analysis_data.get('portfolio_url', 'N/A'),
                    'website_title': analysis_data.get('extracted_website_title', 'N/A'),
                    'projects_count': analysis_data.get('extracted_projects_count', 0),
                    'skills_count': analysis_data.get('extracted_skills_count', 0)
                }
    ...
    def get_user_analysis_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'average_score': round((score_sum / scored_analyses), 1) if scored_analyses > 0 else 0
            }
            
        except Exception as e:
            logger.error(f"Error getting user portfolio analysis statistics: {e}")
            return {
